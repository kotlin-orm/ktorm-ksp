...
public class InterfaceEntityCopyFunGeneratorTest : BaseKspTest() {
    ...
    public fun `generate test`() {
        val (result1, result2) = twiceCompile(
            ...
            object TestBridge {
                public fun callNoArgs(): String {
                    return User(id = null).copy().toString()
                }
                ...
                public fun callAllArgs(): String {
                    return User(username = "luck")
                            .copy(id = null, username = "jack", age = 10, birthday = LocalDate.MIN)
                            .toString()
                }
            }
        """
        ...
        assertThat(noArgs).isEqualTo("User{id=null}")
        val oneArgs = result2.invokeBridge("callOneArgs")
        assertThat(oneArgs).isEqualTo("User{id=null, username=jack}")
        val allArgs = result2.invokeBridge("callAllArgs")
        assertThat(allArgs).isEqualTo("User{username=jack, id=null, age=10, birthday=-999999999-01-01}")
    }
