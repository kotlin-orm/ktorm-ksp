...
private fun createUndefinedValueBySubclassing(cls: Class<*>): Any {
    var subclassName = cls.name + "\$KtormUndefined"
    if (cls.name.startsWith("java.")) {
        subclassName = "\$" + subclassName
    }

    val classLoader = SubclassingClassLoader(cls.classLoader)
    val subclass = Class.forName(subclassName, true, classLoader)
    return unsafe.allocateInstance(subclass)
}

private class SubclassingClassLoader(parent: ClassLoader) : ClassLoader(parent) {
